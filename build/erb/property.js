...
const parser = P.string("#").then(U.Identifier).chain((property) => {
    switch (property.toUpperCase()) {
        ...
        case "DIM": return P.seqMap(U.WS1.then(P.alt(P.regex(/CONST/i), P.regex(/DYNAMIC/i), P.regex(/GLOBAL/i), P.regex(/REF/i), P.regex(/SAVEDATA/i), P.regex(/CHARADATA/i))).many(), U.WS1.then(U.sepBy1(",", U.Identifier, E.expr)), P.alt(P.string("=").trim(U.WS0).then(U.sepBy0(",", E.expr)), P.succeed(undefined)), P.string(",").fallback(null), (prefix, [name, ...size], value) => new Dim(name, "number", prefix, size, value));
        case "DIMS": return P.seqMap(U.WS1.then(P.alt(P.regex(/CONST/i), P.regex(/DYNAMIC/i), P.regex(/GLOBAL/i), P.regex(/REF/i), P.regex(/SAVEDATA/i), P.regex(/CHARADATA/i))).many(), U.WS1.then(U.sepBy1(",", U.Identifier, E.expr)), P.alt(P.string("=").trim(U.WS0).then(U.sepBy0(",", E.expr)), P.succeed(undefined)), P.string(",").fallback(null), (prefix, [name, ...size], value) => new Dim(name, "string", prefix, size, value));

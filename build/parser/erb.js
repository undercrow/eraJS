...
function parseFn(lines, from) {
    ...
    const argParser = U.sepBy0(",", P.seq(X.variable, P.alt(P.string("=").trim(C.WS0).then(C.Int).map((val) => BigInt(val)), P.string("=").trim(C.WS0).then(C.Str), P.string("=").trim(C.WS0).then(X.variable), P.succeed(null))));
    const defParser = P.string("@").then(P.seq(C.Identifier.skip(C.WS0), P.alt(U.wrap("(", ")", argParser), P.string(",").trim(C.WS0).then(argParser), P.succeed([]))));
...
export function parseThunk(lines, from, until) {
    const body = [];
    let index = from;

...
const parser = P.string("#").then(C.Identifier).chain((property) => {
    switch (property.toUpperCase()) {
        case "DEFINE": return P.seqMap(C.WS1, C.Identifier, C.WS1, X.expr, (_1, name, _2, expr) => new Define(name, expr));
        ...
        case "DIM": return P.seqMap(C.WS1.then(P.alt(P.regex(/CONST/i), P.regex(/DYNAMIC/i), P.regex(/GLOBAL/i), P.regex(/REF/i), P.regex(/SAVEDATA/i), P.regex(/CHARADATA/i))).many(), C.WS1.then(U.sepBy1(",", C.Identifier, X.expr)), P.alt(P.string("=").trim(C.WS0).then(U.sepBy0(",", X.expr)), P.succeed(undefined)), P.string(",").fallback(null), (prefix, [name, ...size], value) => new Dim(name, "number", prefix, size, value));
        case "DIMS": return P.seqMap(C.WS1.then(P.alt(P.regex(/CONST/i), P.regex(/DYNAMIC/i), P.regex(/GLOBAL/i), P.regex(/REF/i), P.regex(/SAVEDATA/i), P.regex(/CHARADATA/i))).many(), C.WS1.then(U.sepBy1(",", C.Identifier, X.expr)), P.alt(P.string("=").trim(C.WS0).then(U.sepBy0(",", X.expr)), P.succeed(undefined)), P.string(",").fallback(null), (prefix, [name, ...size], value) => new Dim(name, "string", prefix, size, value));

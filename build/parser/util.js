...
export const UInt = P.alt(P.seqMap(P.regex(/[0-9]+p/i), P.regex(/[0-9]+/), (base, exponent) => parseInt(base) ** parseInt(exponent)), P.regex(/0b/i).then(P.regex(/[0-1]+/)).map((val) => parseInt(val, 2)), P.regex(/0x/i).then(P.regex(/[0-9a-fA-F]+/)).map((val) => parseInt(val, 16)), P.regex(/[0-9]+/).map((val) => parseInt(val, 10)));
...
export function arg2R0(a0, a1) {
    return P.alt(WS1.then(P.seq(a0, P.string(",").trim(WS0).then(a1).fallback(undefined))), WS0.map(() => [undefined, undefined]));
}
...
export function arg5R3(a0, a1, a2, a3, a4) {
    return WS1.then(P.seqMap(a0, P.string(",").trim(WS0).then(a1), P.string(",").trim(WS0).then(a2), P.string(",").trim(WS0).then(a3).fallback(undefined), P.string(",").trim(WS0).then(a4).fallback(undefined), (...arg) => arg));
}

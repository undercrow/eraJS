...
export default class DimDynamic {
    ...
    apply(vm) {
        ...
        if (this.value != null && this.value.length === 0 && this.type === "number") {
            ...
            context.dynamicMap.set(this.name, Value.Int1D(vm.code.data, this.name, value.length).reset(value));
        }
            ...
            context.dynamicMap.set(this.name, Value.Str1D(vm.code.data, this.name, value.length).reset(value));
        }

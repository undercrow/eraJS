...
const PARSER_BRANCH = U.argNR0(P.alt(P.seqMap(C.Int, P.regex(/TO/i).trim(C.WS1).then(C.Int), (from, to) => ({
    type: "range",
    from: BigInt(from),
    to: BigInt(to),
})), P.seqMap(P.regex(/IS/i).then(U.alt("<=", "<", ">=", ">").trim(C.WS0)), X.expr, (op, value) => ({ type: "compare", op, value })), C.Int.map((value) => ({ type: "equal", value: BigInt(value) })), C.Str.map((value) => ({ type: "equal", value }))));

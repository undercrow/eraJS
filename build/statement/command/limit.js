...
export default class Limit extends Statement {
    ...
    *run(vm) {
        ...
        assert.number(value, "1st Argument of LIMIT should be an integer");
        const min = minExpr.reduce(vm);
        assert.number(min, "2nd Argument of LIMIT should be an integer");
        const max = maxExpr.reduce(vm);
        assert.number(max, "3rd Argument of LIMIT should be an integer");

...
export default class CsvTalent extends Statement {
	...
	public *run(vm: VM) {
		...
		assert.number(num, "1st Argument of CSVTALENT should be an integer");
		const index = indexExpr.reduce(vm);
		assert.number(index, "2nd Argument of CSVTALENT should be an integer");

		const character = vm.code.data.character.get(num);
		assert.cond(character != null, `Character #${num} does not exist`);

		const result = character.talent.get(index) ?? 0;

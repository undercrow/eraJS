...
export default class Method extends Statement {
	...
	public *run(vm: VM) {
		...
		switch (this.name) {
			case "ABS": result = abs(vm, arg); break;
			case "BARSTR": result = barStr(vm, arg); break;
			case "CSVABL": result = csvAbl(vm, arg); break;
			case "CSVBASE": result = csvBase(vm, arg); break;
			case "CSVCALLNAME": result = csvCallname(vm, arg); break;
			case "CSVCFLAG": result = csvCflag(vm, arg); break;
			case "CSVCSTR": result = csvCstr(vm, arg); break;
			case "CSVEQUIP": result = csvEquip(vm, arg); break;
			case "CSVEXP": result = csvExp(vm, arg); break;
			case "CSVJUEL": result = csvJuel(vm, arg); break;
			case "CSVMARK": result = csvMark(vm, arg); break;
			case "CSVMASTERNAME": result = csvMastername(vm, arg); break;
			case "CSVNAME": result = csvName(vm, arg); break;
			case "CSVNICKNAME": result = csvNickname(vm, arg); break;
			case "CSVRELATION": result = csvRelation(vm, arg); break;
			case "CSVTALENT": result = csvTalent(vm, arg); break;
			case "EXISTCSV": result = existCsv(vm, arg); break;
			case "FINDCHARA": result = findChara(vm, arg); break;
			case "FINDLASTCHARA": result = findLastChara(vm, arg); break;
			case "GETBGCOLOR": result = getBgColor(vm, arg); break;
			case "GETBIT": result = getBit(vm, arg); break;
			case "GETCHARA": result = getChara(vm, arg); break;
			...
			case "GROUPMATCH": result = groupMatch(vm, arg); break;
			case "INRANGE": result = inRange(vm, arg); break;
			case "LIMIT": result = limit(vm, arg); break;
			case "LINEISEMPTY": result = lineIsEmpty(vm, arg); break;
			case "MATCH": result = match(vm, arg); break;
			case "MAX": result = max(vm, arg); break;
			case "MAXARRAY": result = maxArray(vm, arg); break;
			case "MIN": result = min(vm, arg); break;
			case "MINARRAY": result = minArray(vm, arg); break;
			case "POWER": result = power(vm, arg); break;
			case "RAND": result = rand(vm, arg); break;
			case "SIGN": result = sign(vm, arg); break;
			case "SQRT": result = sqrt(vm, arg); break;
			case "STRLENS": result = strLenS(vm, arg); break;
			case "STRLENSU": result = strLenSU(vm, arg); break;
			case "SUMARRAY": result = sumArray(vm, arg); break;
			case "TOINT": result = toInt(vm, arg); break;
